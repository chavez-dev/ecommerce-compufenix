$(document).ready(function(){
    ...
    $("#agregarEmpleado").click(function(){
        ...
        $('.modal-title').text("REGISTRO DE CLIENTE");
    ...
    var dataTable = $('#tablaUsuarios').DataTable({
        ...
        "order":[],
        "ajax":{
            url: '../../../backend/consultas/listas/lista_clientes.php',
            type: 'POST',
        },
        "columnDefs":[{
            "targets":[6,4],
    ...
    $(document).on('submit','#formulario-empleado',function(event){
        ...
        if (nombre != '' && nro_documento != '') {
            $.ajax({
                url: "../../../backend/consultas/CRUDS/CRUD_cliente.php",
                method: "POST",
    ...
    $(document).on('click', '.editar', function(){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                desbloquearCamposYBotones();
                $('#tipo_documento').val(data.tipo_cliente);
...
function bloquearCamposYBotones() {
    $('#tipo_documento, #nro_documento, #nombre, #nacionalidad, #email, #celular, #direccion')
        .prop('disabled', true);
